...
vuser_init()
{
	...
	lr_save_string(lr_eval_string ("LSP_{USER_ID}"), "MTAPROJECT");
	lr_save_string( lr_eval_string ("w93e87d70"), "ACCOUNT");
	lr_save_string( lr_eval_string ("https://devdi-{ACCOUNT}.dispatcher.int.sap.hana.ondemand.com"), "URL");
	lr_save_string( lr_eval_string ("http://10.26.156.125:50000/saml2/idp/sso"), "SAML_REDIRECT_URL");
	lr_save_string( lr_eval_string ("100"), "ThinkTime");
	
lr_start_transaction("Authentication_MTA");
...
/*Correlation comment - Do not change!  Original value='oucrssnlobxfazsqefzdsjozd' Name ='CorrelationParameter_1' Type ='ResponseBased'*/
	...
	web_url("{URL}", 
        ...
        lr_end_transaction("Authentication_MTA", LR_AUTO);
...
//	
	...
	//only in case workspace is already existing
